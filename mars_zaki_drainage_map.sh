#!/bin/bash
#
# Global map of Zaki et al.'s Large_drainage_systems_reprojected.shp (16
# large drainage systems) over a MOLA hillshade backdrop, built with GMT.
# Uniform bright-orange fill, mostly opaque (t=15) so it reads clearly
# against the grayscale backdrop -- no classification; only fields are X,
# Y, Area, with too few polygons (16) for a meaningful classed choropleth
# anyway.
#
...
# This also means GMT's own geographic map-scale (-L) isn't available, so
# the scale bar is drawn manually from the Mars sphere radius.
#
# Requires GMT 6 (conda install -c conda-forge gmt).
#
# Usage: ./mars_zaki_drainage_map.sh
# Output: <OUT_DIR>/mars_zaki_drainage_systems_map.png
...
SHP_ZIP="$BASE_DIR/Jack, Ze-Wen summer project files/Zaki et al. Data/Large_drainage_systems_reprojected.zip"
SHP_NAME="Large_drainage_systems_reprojected.shp"
...
if [[ ! -f "$SHP_ZIP" ]]; then
    echo "Zaki drainage systems zip not found: $SHP_ZIP" >&2
    exit 1
fi

SHP="$ZAKI_DIR/$SHP_NAME"
if [[ ! -f "$SHP" ]]; then
    echo "Extracting Zaki et al. drainage systems shapefile..."
    unzip -oq "$SHP_ZIP" -d "$ZAKI_DIR"
fi
...
N_SYSTEMS=$(ogrinfo -al -so "$SHP" | awk -F': ' '/Feature Count/{print $2}')
...
# --- render ---
MAP_NAME="mars_zaki_drainage_systems_map"
...
gmt begin "$MAP_NAME" png
    gmt basemap -R"$REGION" -JX24c/12c -Bxafg30+l"Longitude" -Byafg30+l"Latitude" \
        -BWSen+t"Zaki et al. Large Drainage Systems"
    gmt grdimage "$DOWNSAMPLED" -C"$GRAY_CPT" -I"$HILLSHADE"
    gmt plot "$SHP" -G"$FILL" -t"$FILL_TRANSPARENCY" -W0.75p,"$OUTLINE"
